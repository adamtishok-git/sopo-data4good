...
function blockStyle(color, isEdited, isSelected) {
  return {
    fillColor:   color,
    fillOpacity: 0.65,
    color:       isSelected ? '#000' : isEdited ? '#555' : '#555',
    weight:      isSelected ? 2.5 : isEdited ? 2 : 0.6,
    dashArray:   isEdited && !isSelected ? '5,3' : null,
  };
}

export default function MapView({ scenarioData, assignments, editedBlocks, selectedBlockId, onBlockClick }) {
  const mapElRef    = useRef(null);
  const mapRef      = useRef(null);
  const layersRef   = useRef({});
  const clickCbRef  = useRef(onBlockClick);

  // Keep callback ref current without reinitializing map
  useEffect(() => { clickCbRef.current = onBlockClick; });

  // Initialize map once on mount
  useEffect(() => {
    const { openSchools, schools, blocks } = scenarioData;
    ...
    blocks.forEach(block => {
      const school = assignments[block.id];
      const color  = schools[school]?.color || '#ccc';
      const layer  = L.geoJSON(block.geometry, { style: blockStyle(color, false, false) });

      layer.on('click', () => clickCbRef.current(block));
      layer.on('mouseover', e => {
        const sid  = assignments[block.id] || school;
        const wd   = block.walkDists[sid];
        const dd   = block.driveDists[sid];
        const walk = wd !== null && wd <= WALK_RADIUS;
        const fmt  = m => m !== null ? (m / 1609.34).toFixed(2) + ' mi' : 'N/A';
        layer.bindTooltip(
          `<b>${block.id}</b><br>` +
          `Pop: ${block.population} &nbsp;·&nbsp; Students: ${block.students.toFixed(1)}<br>` +
          `Assigned: ${sid}<br>` +
          `Walk: ${fmt(wd)} ${walk ? '🚶' : ''} &nbsp;·&nbsp; Drive: ${fmt(dd)} ${!walk ? '🚗' : ''}`,
          { sticky: true }
    ...
    // Walk-radius circles + school markers
    openSchools.forEach(sid => {
      const s = schools[sid];
      L.circle([s.lat, s.lng], {
        radius: WALK_RADIUS, color: s.color, weight: 1.5,
        dashArray: '6', fill: false,
      }).addTo(map);

      L.circleMarker([s.lat, s.lng], {
        radius: 9, color: '#fff', weight: 2,
        fillColor: s.color, fillOpacity: 1,
      }).bindTooltip(`${sid} Elementary`, { permanent: false }).addTo(map);
    });
  ...
  // Update polygon styles when assignments or selection change
  useEffect(() => {
    ...
    blocks.forEach(block => {
      ...
      const color  = schools[sid]?.color || '#ccc';
  ...
  }, [assignments, editedBlocks, selectedBlockId]); // eslint-disable-line react-hooks/exhaustive-deps
